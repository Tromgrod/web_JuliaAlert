...
{
    [Bo(DisplayName = "Анализ по моделям MD", CustomPage = true)]
    public class ModelList_LocalSales : ReportBase
    {
        public override string GetLink() => string.Empty;

        public override string GetAction() => $"open_report_popup('SpecificProduct_ModelList_LocalSales', 'UniqueProduct', 'JuliaAlert.Models.Objects', {UniqueProduct.Id})";
        ...
        public override Dictionary<long, ItemBase> PopulateReport(SqlConnection conn, ItemBase item, int iPagingStart, int iPagingLen, string sSearch, List<SortParameter> SortParameters, LIB.BusinessObjects.User sUser, out long idisplaytotal, out Dictionary<string, decimal> ColumsSum)
        {
            ...
            if (item != null && item is ModelList_LocalSales)
            ...
            var modelLists = new Dictionary<long, ItemBase>();

            foreach (DataRow dr in dataRows)
            {
                if (rowCounter >= iPagingStart && iPagingLen > 0)
                {
                    var modelList = new ModelList_LocalSales
                    {
                        ...
                        {
                            Name = dr[nameof(TypeProduct) + nameof(TypeProduct.Name)].ToString()
                        },
                    ...
                    modelLists.Add(rowCounter, modelList);
