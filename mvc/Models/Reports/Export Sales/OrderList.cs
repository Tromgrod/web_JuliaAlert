...
namespace JuliaAlert.Models.Reports
{
    ...
    public class OrderList : ReportBase
    {
        ...
        [Common(DisplayName = "Канал продаж", _Sortable = true, _Searchable = true),
         Template(Mode = Template.ParentDropDown),
         Db(PopulateDropDown = nameof(SalesChannel.PopulateByWord)),
         Access(DisplayMode = DisplayMode.Simple | DisplayMode.Excell | DisplayMode.Search)]
        ...
        [Common(DisplayName = "Заказ $", EditTemplate = EditTemplates.DecimalNumberRange, _Sortable = true, _Searchable = true, DecimalRound = 2, TotalSum = true),
        ...
        [Common(DisplayName = "Продажи $", EditTemplate = EditTemplates.DecimalNumberRange, _Sortable = true, _Searchable = true, DecimalRound = 2, TotalSum = true),
         Access(DisplayMode = DisplayMode.Simple | DisplayMode.Excell | DisplayMode.Search, VisibleFor = (long)BasePermissionenum.MoneyInReportsAccess)]
        public DecimalNumberRange SalesSUM { get; set; }

        [Common(DisplayName = "Прибыль $", EditTemplate = EditTemplates.DecimalNumberRange, _Sortable = true, _Searchable = true, DecimalRound = 2, TotalSum = true),
         Access(DisplayMode = DisplayMode.Simple | DisplayMode.Excell | DisplayMode.Search, VisibleFor = (long)BasePermissionenum.MoneyInReportsAccess)]
        public DecimalNumberRange RealSUM { get; set; }
            ...
            foreach (DataRow dr in dataRows)
            {
                if (rowCounter >= iPagingStart && iPagingLen > 0)
                {
                    ...
                    var list = new OrderList
                    {
                        ...
                        OrderNumber = dr[nameof(OrderNumber)].ToString(),
                        InvoiceNumber = dr[nameof(InvoiceNumber)].ToString(),
                        ...
                        RealSUM = new DecimalNumberRange() { From = Convert.ToDecimal(dr[nameof(RealSUM)]), PostFix = CurrencyName },
